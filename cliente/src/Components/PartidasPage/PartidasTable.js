...
function PartidasTable() {
  ...
  useEffect(() => {
    ...
    axios
      .get('https://localhost:3000/partidas') // Rota para buscar todas as partidas
    ...
    axios
      .get(`https://localhost:3000/partidas/atribuidas/${userId}`) // Corrigido a interpolação da URL
  ...
  const handleDelete = (partidaId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://localhost:3000/partidas/${partidaId}`) // Corrigido a interpolação da URL
          .then(() => {
