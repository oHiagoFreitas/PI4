...
const RecentReportsTable = () => {
    ...
    const fetchReports = async () => {
        try {
            const response = await axios.get('https://localhost:3000/relatorios/pendentes');
    ...
    const handleApprove = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.put(`https://localhost:3000/relatorios/${id}/aprovar`);
    ...
    const handleReject = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                await axios.put(`https://localhost:3000/relatorios/${id}/rejeitar`);
